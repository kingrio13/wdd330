...
class QuakesView {
  ...
  renderBack(){
    const rio=document.querySelector('#quakeContainer')
    const backbtn=document.createElement('a');
    backbtn.setAttribute('id','backbtn');
    backbtn.setAttribute('class','back btn cool')
    backbtn.innerText="back";
    rio.appendChild(backbtn);
    ...
    element.innerHTML = quakeProperties
      .map(item => {
        if (item[0] === 'time' || item[0] === 'updated') {
          return `<li class="specific">${item[0]}: ${new Date(item[1])}</li>`;
        } else return `<li class="specific">${item[0]}: ${item[1]}</li>`;
      })
      .join('');
